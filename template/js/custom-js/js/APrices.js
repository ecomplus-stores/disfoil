...
export default {
  ...
  mounted () {
    if (this.isProduct) {
      this.$nextTick(() => {
        const productPrices = document.querySelector('.product__prices');
        if (productPrices) {
          const label = productPrices.querySelector('.prices__discount .prices__discount-label');
          if (label) {
            label.textContent = 'no pix/boleto';
